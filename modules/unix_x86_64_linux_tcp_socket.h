...
/*
*   todo @unix_x86_64_linux_tcp_socket_connect make connection to @ip:@port
*/
int unix_x86_64_linux_tcp_socket_connect(const char* ip, int port);
...
int unix_x86_64_linux_tcp_socket_connect(const char* ip, int port)
{
    ...
    connect(sd, (struct sockaddr *) &si, sizeof(si));
    return sd;
}
